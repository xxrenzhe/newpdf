...
class TextArtElement extends TextElement {
    init() {
        super.init();
        this.dataType = 'textArt';
    }
    ...
    async insertToPDF() {
        ...
        if(this.attrs.isGradient == 'true'){
            if(this.attrs.gradientType == 'right'){
                var gradient = ctx.createLinearGradient(0, 0, canvas.width, 0);
                gradient.addColorStop("0", this.attrs.gradientColor);
                gradient.addColorStop("1", this.attrs.gradientColorTwo);
        ...
        if(this.attrs.isGradient == 'false'){
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if(this.attrs.isGradient == 'false'){
                ctx.strokeText(lines[i], offsetX, offsetY - (this.attrs.underline ? 10 : 15), canvas.width);
            }
